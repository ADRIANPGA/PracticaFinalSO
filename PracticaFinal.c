...
void *accionesSolicitud(void *ptr){ 
	...
	printf("Solicitud_%d: ", identificadorSolicitud+1);
				...
				if(listaDeUsuarios[i].atendido==0){
					...
					switch(tipoSol){
						...
						if(esperar>90){
							numSolicitudes--;
							pthread_exit(NULL);
							printf("La solicitud se va por cansarse de esperar \n");
						...
						if(esperar>70){
							numSolicitudes--;
							printf("La solicitud de va por cansarse de esperar \n");
					...
					if(errorSistema >= 50){
						...
						while(TRUE){
							...
							if(contadorActividad==4){
								...
								printf("La solicitud esta preparada para participar en la actividad social \n");
								escribirEnLog(idSolicitud, mensajeLog);
								...
								printf("La solicitud deja de participar en la actividad social\n");
				...
				if(listaDeUsuarios[i].atendido==0){
					...
					switch(tipoSol){
						...
						if(esperar>90){
							...
							printf("La solicitud sale debido a un gran tiempo de espera\n");
							escribirEnLog(idSolicitud, mensajeLog);
						}else{
							esperar = calculaAleatorios(0,100);
                                    /* Si esperar tiene un valor mayor que 85 se expulsa la solicitud por error en la app, sin avisar, sin log */
						...
						if(esperar>70){                                    
							numSolicitudes--;
							pthread_exit(NULL);
							printf("La solicitud sale debido a no ser fiable\n");
					...
					errorSistema = calculaAleatorios(0,100);
					if(errorSistema >= 50){
						printf("El usuario no puede pertenecer a una actividad cultural. Es expulsado!\n");
